...
RES=$(awk '{ if ($0 ~ "Service Name: ") { print $3;getline;getline;getline;print $2;getline;print $2;getline;getline;getline;getline;getline;getline;getline;print $4 } }' ~/parse)
RESA=(${RES})
...
do
  if (( $((i % 4 )) == 0 )); then
       if [ ! -z ${RESA[$i]} ]; then
           ...
           HOST=${RESA[$i+1]}
           PATH=${RESA[$i+2]}
           PORT=$(echo ${RESA[$i+3]} | /usr/bin/awk '{split($0,a,":"); print a[2]}')
